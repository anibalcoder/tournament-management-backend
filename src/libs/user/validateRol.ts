...
export const validateRol = async ({
  ...
  if (currentRole === users_role.club_owner) {
    if (!dni || dni.trim() === '') {
      return { error: 'El DNI es obligatorio para dueños de club', status: 400 };
    }
  ...
  if (currentRole === users_role.competitor) {
    if (!club_id) {
      return { error: 'El club_id es obligatorio para competidores', status: 400 };
    }

    const clubExists = await prisma.clubs.findUnique({ where: { id: club_id } })
